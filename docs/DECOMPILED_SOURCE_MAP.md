...
| Minimap zoom (HUD minimap scale, speed auto-zoom) | CommonMapBar (TriggerByMe offset = -pos.xz*MapRatio*0.55 into maproot@t/dec_sketch@t@img + map_sketch@t; StatusPanel and VehicleStatusPanel each own one), MiniMapSpotWidget (SetData base scale 1.15/1.3, RefreshMiniMapPosition), MiniMapSystem (DataModule) distance/trackDistance fields, MovementComponent._realSpeed, VehicleManager.GetSelfEntityVehicle → VehicleComponent.RunForwardMaxSpeed/controller → VehicleControllerBase.moveSpeed/_locomotion (Normal vs Remote) | MiniMapZoomFeature.cs (+ HeartopiaComplete.UguiMiniMapContent.cs) | A |
