...
## BuildModule access (`PadBuildHotkeyFeature.cs`)

The Pad build hotkeys that gave this file its name were removed on 2026-09-24 (the game's own
build shortcuts replace them, rebindable in Settings → Game Keys). What remains is the shared
`BuildModule` access every build feature uses: `TryGetPadBuildAuraModule` (AuraMono
`Managers.GetModule(Type)`), `get_SubState` for `TryGetPadBuildAuraSubState`, and
`TryFindPadBuildPanelRoot`.

Resolution: class via `FindAuraMonoClassInImages("XDTGUI.Module.Build", "BuildModule",
